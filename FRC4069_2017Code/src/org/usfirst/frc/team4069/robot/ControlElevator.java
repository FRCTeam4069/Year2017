...
public class ControlElevator
{
  ...
  {
    /*
     * updateDirection(); // if back button is pressed once, toggle between main/second speed if(Robot.InputSystem.Back_Button_Control_Stick_Once){ useSecondSpeed = !useSecondSpeed; } if(Math.abs(mRobot.mRobotSpeed) >= 0.05 || InputSystem.Dpad_Up_Control_Stick ||
     * InputSystem.Dpad_Down_Control_Stick){ // if robot is moving or dpad up/down is pressed, enable elevator if(mEnabled == 0){ Enable(); } } else{ // if robot is stationary and dpad up/down aren't pressed, disable elevator if(mEnabled == 1){ Disable(); } } if (mEnabled==1) {
     * //elevatorTalon.set(getElevatorSpeed()); } else{ //elevatorTalon.set(0); }
     */
    if (mEnabled == 1)
    {
      elevatorTalon.set(getElevatorSpeed());
